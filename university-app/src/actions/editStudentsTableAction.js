...
export function getStudents() { // из этой функции возвращаем другую функцию, которая принимает функцию dispatch. Делается для того, чтобы можно было генерировать несколько действий в рамках одной функции
  return dispatch => {  // вызываем функцию до отправки запроса
    ...
    return axios.get('http://localhost:8080/university/students/')  // возвращаем результат вызова функции
...
export function addStudent(firstName, lastName) {
  return axios.post('http://localhost:8080/university/students/', {firstName, lastName})
...
export function deleteStudent(id) {
  return axios.delete(`http://localhost:8080/university/students/${id}`)
...
export function updateStudent(id, firstName, lastName) {
  return axios.put(`http://localhost:8080/university/students/${id}`, firstName, lastName)
    .then(response => response.data)
    .then(student => ({
      type: UPDATE_STUDENT,
      student
